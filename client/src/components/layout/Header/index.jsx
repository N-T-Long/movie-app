...
import { Button, Navbar, Form, FormControl, NavDropdown, Container, Nav } from 'react-bootstrap';
                ...
                <Navbar.Collapse id="responsive-navbar-nav">
                    <Nav className="me-auto">
                        <NavDropdown menuVariant="dark" title="Phim lẻ" id="phimle-dropdown" className='nav-dropdown-item' >
                            {
                                ...
                                phimle.map(item => {
                                    return <Link to={`/phim-le/${item.link}`} className="dropdown-item">{item.name}</Link>
                                })
                            }
                        </NavDropdown>
                        <NavDropdown menuVariant="dark" title="Phim bộ" id="phimbo-dropdown" className="nav-dropdown-item">
                            {
                                phimbo.map(item => {
                                    return <Link to={`/phim-bo/${item.link}`} className="dropdown-item">{item.name}</Link>
                                })
                            }
                        </NavDropdown>
                        <NavDropdown menuVariant="dark" title="Quốc gia" id="quociga-dropdown"className="nav-dropdown-item">
                            {
                                quocgia.map(item => {
                                    return <Link to={`/quoc-gia/${item.link}`} className="dropdown-item">{item.name}</Link>
                                })
                            }
                        </NavDropdown>
                        <NavDropdown menuVariant="dark" title="Thể loại" id="theloai-dropdown"className="nav-dropdown-item">
                            {
                                theloai.map(item => {
                                    return <Link to={`/the-loai/${item.link}`} className="dropdown-item">{item.name}</Link> 
                                })
                            }
                        </NavDropdown>
                    </Nav>
